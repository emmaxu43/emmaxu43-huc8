...
def RF(bain_ID, X):
  ...
  return Rscore, clf, scores, yt, Xv, yv
...
## Generate RFs
Rscore, clf, scores, yt, Xv, yv= RF(basin_ID, X)
...
for i in hrchy:
  ...
  # print(i)
  for j in basin_idx:
    tmp= np.asarray(ssurgo_class[j])
    ssurgo[i][j]= np.unique(tmp[:,n])
...
for i in hrchy:
  intersect_class[i]={}
  intersect_pruned_prob[i]={} # before normalizing
